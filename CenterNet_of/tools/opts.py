...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--task', default='ctdet',
                             help='ctdet | ddd | multi_pose | exdet | ttfdet | mot')
    self.parser.add_argument('--dataset', default='coco',
                             help='coco | kitti | coco_hp | pascal | seaships')
    self.parser.add_argument('--exp_id', default='coco_dla')
    self.parser.add_argument('--test', action='store_true')
    self.parser.add_argument('--demo', default='./data/19064748793_bb942deea1_k.jpg',
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')
    self.parser.add_argument('--load_model', default="./model/checkpoint_ep37",
    ...
    self.parser.add_argument('--lr_step', default=[90, 120], nargs='+', type=int,
    ...
    self.parser.add_argument('--num_classes', type=int, default=80,
                             help='coco dataset number classes')
    # ttfdet
    self.parser.add_argument('--alpha', default='0.54', help='')
    self.parser.add_argument('--beta', default='0.54', help='')
    # multi_pose
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'mot':
      ...
      opt.img_size = (1088, 608)
        ...
        opt.heads = {'hm': opt.num_classes,
                    'wh': 2 if opt.cat_spec_wh else 2 * opt.num_classes}
        if opt.reg_offset:
            opt.heads.update({'reg': 2})
    else:
